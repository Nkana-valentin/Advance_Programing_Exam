...
public:
    ...
    /*     void erase(const K &x)
    {
        ...
        if (loc == _root.get())
        {
            ...
            else if (!(!loc->_left) != !(!loc->_right))
            {
                ...
                if (loc->_right)
                {
                    ...
                    // child is the right one
                    _root.get(loc->_right.release());
                    loc->_right->_parent = nullptr;
                }
                else
                {
            ...
            else
            {
                ...
                auto succ = (++it).current;
                auto DN = std::unique_ptr<Node<const K, V>>(new Node<const K, V>(succ->_data, nullptr));
                DN->_left.reset(loc->_left.release());
                DN->_left->_parent = DN.get();
                if (succ == loc->_right.get())
                {
                ...
                else
                {
                    ...
                    // the successor is not the next one

                    DN->_right.reset(loc->_right.release());
                    DN->_right_parent = DN.get();
                    if (succ->_right)
                    {
        ...
        else
        {
            ...
            // the node to be selected is not the root
            if (!(loc->_left) && !(loc->_right))
            {
            ...
            else
            {
                ...
                ++it;
                Node<const K, V> *succ{it.current};

                if (succ != loc->_right.get())
                {
                    auto DNP{loc->_parent};
                    // DNP is the pointer to the parent of the node to be deleted
                    auto DN{std::unique_ptr<Node<const K, V>>(new Node<const K, V> > (succ->_data, DNP))};
                    // DN is the unique pointer to the newly created node which contains the successor's data
                    erase(succ->_data.first);

                    DN->_left.reset(loc->_left.release());
                    // loc points to the node to be deleted
                    DN->_left->_parent = DN.get();
                    DN->_right.reset(loc->_right.release());
                    DN->_right->_parent = DN.get();
    ...
    /* K &tree_minimum(const std::unique_ptr<Node<K, V>> &curr) noexcept
    {

        //auto curr = std::make_unique<Node<K, V>>(tree._root, nullptr);
        if (curr && curr->_left == nullptr)
            return curr->_data.first;
        return tree_minimum(curr->_left);
    }
 */
    template <typename T>
    V &operator[](T &&key);
